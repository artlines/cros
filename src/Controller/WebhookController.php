...
class WebhookController extends AbstractController
{
    ...
    private function _start()
    {
        ...
        $repo = $this->getDoctrine()->getRepository('App:TgChat');
    ...
    public function _contactList($page = 1)
    {
        ...
        $orgRepo = $em->getRepository('App:Organization');
    ...
    private function _showHalls($date)
    {
        ...
        $all_halls = $this->getDoctrine()->getRepository("App:Hall")->findAll();
    ...
    private function _showLectures($date, $hallId, $page, $lecture_action = null, $lecture_id = null, $info_action = null)
    {
        ...
        if (isset($lecture_action, $lecture_id) && $lecture_action == 'info') {
            ...
            /** @var LectureRepository $lectureRepo */
            $lectureRepo = $this->getDoctrine()->getRepository("App:Lecture");
    ...
    {
        $lecture = $this->getDoctrine()->getRepository('App:Lecture')->find($lectureId);
        if (!$lecture) {
    ...
    {
        $hall = $this->getDoctrine()->getRepository('App:Hall')->find($hallId);
        if (!$hall) {
    ...
    private function _findTgChat()
    {
        ...
        $tgChat = $this->getDoctrine()->getRepository('App:TgChat')->findOneBy(['chatId' => $tgChatId]);
        if (!$tgChat) {
            $tgChat = new TgChat();
